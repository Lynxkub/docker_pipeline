...
def main():
    ...
    home_dir = {}
    zipcodes = Variable.get("zipcodes", deserialize_json=True)
    print(zipcodes)
    wisconsin_zipcodes = zipcodes["wisconsin"]
    for zip in wisconsin_zipcodes:
    ...
    df['orig_listing_price'] = df['price']
    df['list_price_change'] = np.nan
    print(df.head())
    df.to_parquet(f's3://housing-data-docker/redfin-data/raw_data/meta_load_dt={current_timestamp}/home_data.parquet', storage_options = {
        "key": access_key,
